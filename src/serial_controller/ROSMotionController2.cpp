 ...
 *
 */
ROSMotionController2::ROSMotionController2(std::string name_node, const ros::NodeHandle& nh, ParserPacket* serial, ServiceSerial* service_serial)
: nh_(nh) {
    name_node_ = name_node; // Initialize node name
    this->serial_ = serial; // Initialize serial port
    service_serial_ = service_serial;
    //    serial_->setAsyncPacketCallback(&ROSMotionController2::actionAsync, this);

    //Open Publisher
    //- Measure
    velocity_mis_pub_ = nh_.advertise<serial_bridge::Velocity>("/" + name_node + "/" + measure_string + "/" + velocity_string, 1000,
            boost::bind(&ROSMotionController2::connectCallback, this, _1));
    //- Command receive
    ...
    //Open Subscriber
    //- Command
    pose_sub_ = nh_.subscribe("/" + name_node + "/" + command_string + "/" + pose_string, 1, &ROSMotionController2::poseCallback, this);
    velocity_sub_ = nh_.subscribe("/" + name_node + "/" + command_string + "/" + velocity_string, 1, &ROSMotionController2::velocityCallback, this);
    enable_sub_ = nh_.subscribe("/" + name_node + "/" + command_string + "/" + enable_motors, 1, &ROSMotionController2::enableCallback, this);
    //-- Conventional (Using TF, NAV)
    pose_estimate_sub_ = nh_.subscribe("/" + name_node + "/" + command_string + "/" + odometry_string, 1, &ROSMotionController2::pose_tf_Callback, this);

    //Open Service
    pid_update_srv_ = nh_.advertiseService("/" + name_node + "/" + update_pid_string, &ROSMotionController2::pid_update_Callback, this);
    parameter_update_srv_ = nh_.advertiseService("/" + name_node + "/" + update_parameter_string, &ROSMotionController2::parameter_update_Callback, this);
    constraint_update_srv_ = nh_.advertiseService("/" + name_node + "/" + update_constraint_string, &ROSMotionController2::constraint_update_Callback, this);
    process_update_srv_ = nh_.advertiseService("/" + name_node + "/" + update_process_string, &ROSMotionController2::process_update_Callback, this);
    convert_velocity_srv_ = nh_.advertiseService("/" + name_node + "/" + convert_string, &ROSMotionController2::convert_Callback, this);
    ...
    //Timer
    timer_ = nh_.createTimer(ros::Duration(1 / rate), &ROSMotionController2::timerCallback, this, false, false);
}

void ROSMotionController2::actionAsync(packet_t packet) {
    ROS_INFO("ROS Motion Controller Async");
}

ROSMotionController2::~ROSMotionController2() {
}

void ROSMotionController2::quit(int sig) {
    ROS_INFO("Stop timer");
    timer_.stop();
}

void ROSMotionController2::loadParameter() {
...
void ROSMotionController2::timerCallback(const ros::TimerEvent& event) {
...
packet_t ROSMotionController2::updatePacket() {
    std::vector<information_packet_t> list_send;
...
void ROSMotionController2::connectCallback(const ros::SingleSubscriberPublisher& pub) {
    ROS_INFO("Connect: %s - %s", pub.getSubscriberName().c_str(), pub.getTopic().c_str());
    timer_.start();
}

void ROSMotionController2::velocityCallback(const serial_bridge::Velocity::ConstPtr &msg) {
...
void ROSMotionController2::enableCallback(const serial_bridge::Enable::ConstPtr &msg) {
...
void ROSMotionController2::updateOdom(const serial_bridge::Pose* pose) {
    double space;
...
void ROSMotionController2::pose_tf_Callback(const geometry_msgs::PoseWithCovarianceStamped::ConstPtr& msg) {
...
void ROSMotionController2::poseCallback(const serial_bridge::Pose::ConstPtr &msg) {
    //  ROS_INFO("POSE CALLBACK");
    updateOdom(msg.get());
}

pid_control_t ROSMotionController2::get_pid(std::string name) {
    pid_control_t pid;
    double temp;
...
parameter_motors_t ROSMotionController2::get_parameter() {
...
constraint_t ROSMotionController2::get_constraint() {
...
process_t ROSMotionController2::get_process(std::string name) {
    process_t process;
    int temp;
    process.idle = 0;
...
bool ROSMotionController2::parameter_update_Callback(std_srvs::Empty::Request&, std_srvs::Empty::Response&) {
...
bool ROSMotionController2::process_update_Callback(serial_bridge::Update::Request &req, serial_bridge::Update::Response&) {
...
bool ROSMotionController2::pid_update_Callback(serial_bridge::Update::Request &req, serial_bridge::Update::Response&) {
    std::string name = req.name;
    pid_control_t pid;
    std::vector<information_packet_t> list_send;
    ROS_INFO("PID UPDATE");
...
bool ROSMotionController2::constraint_update_Callback(std_srvs::Empty::Request&, std_srvs::Empty::Response&) {
    constraint_t constraint = get_constraint();
...
bool ROSMotionController2::convert_Callback(serial_bridge::Convert::Request &req, serial_bridge::Convert::Response & msg) {
...
void ROSMotionController2::parser(ros::Duration duration, std::vector<information_packet_t> serial_packet) {
    serial_bridge::Pose pose;
...
void ROSMotionController2::sendJoint(serial_bridge::Motor motor_left, serial_bridge::Motor motor_right) {
...
void ROSMotionController2::sendOdom(serial_bridge::Velocity velocity, serial_bridge::Pose pose) {
