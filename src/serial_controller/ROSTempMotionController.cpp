/* 
 * File:   ROSTempMotionController.cpp
 * Author: raffaello
 * 
...
ROSTempMotionController::ROSTempMotionController(std::string name_node, const ros::NodeHandle& nh, ParserPacket* serial)
: ROSController(name_node, nh, serial) {
    serial->addCallback(&ROSTempMotionController::motionPacket, this, HASHMAP_MOTION);
    addVectorPacketRequest(&ROSTempMotionController::updatePacket, this);
    addParameterPacketRequest(&ROSTempMotionController::addParameter, this);
    addTimerEvent(&ROSTempMotionController::timerEvent, this);
    ...
    //Open Subscriber
    //- Command
    sub_pose = nh_.subscribe(name_node + "/" + command_string + "/pose", 1, &ROSTempMotionController::poseCallback, this);
    sub_velocity = nh_.subscribe(name_node + "/" + command_string + "/velocity", 1, &ROSTempMotionController::velocityCallback, this);
    sub_enable = nh_.subscribe(name_node + "/" + command_string + "/enable", 1, &ROSTempMotionController::enableCallback, this);
    //-- Conventional (Using TF, NAV)
    sub_pose_estimate = nh_.subscribe(name_node + "/" + command_string + "/odometry", 1, &ROSTempMotionController::poseTFCallback, this);

    //Open Service
    srv_pid = nh_.advertiseService(name_node + "/pid", &ROSTempMotionController::pidServiceCallback, this);
    srv_parameter = nh_.advertiseService(name_node + "/parameter", &ROSTempMotionController::parameterServiceCallback, this);
    srv_constraint = nh_.advertiseService(name_node + "/constraint", &ROSTempMotionController::constraintServiceCallback, this);
}

ROSTempMotionController::~ROSTempMotionController() {
...
void ROSTempMotionController::addParameter(std::vector<information_packet_t>* list_send) {
    //Constraint
...
void ROSTempMotionController::motionPacket(const unsigned char& command, const abstract_packet_t* packet) {
    switch (command) {
        case CONSTRAINT:
...
void ROSTempMotionController::timerEvent(const ros::TimerEvent& event) {
...
void ROSTempMotionController::updatePacket(std::vector<information_packet_t>* list_send) {
...
void ROSTempMotionController::velocityCallback(const serial_bridge::Velocity::ConstPtr &msg) {
    velocity_t velocity;
...
void ROSTempMotionController::enableCallback(const serial_bridge::Enable::ConstPtr &msg) {
...
void ROSTempMotionController::poseCallback(const serial_bridge::Pose::ConstPtr &msg) {
    saveOdometry(msg.get());
}

void ROSTempMotionController::poseTFCallback(const geometry_msgs::PoseWithCovarianceStamped::ConstPtr& msg) {
...
void ROSTempMotionController::saveOdometry(const serial_bridge::Pose* pose) {
...
void ROSTempMotionController::sendOdometry(const serial_bridge::Velocity* velocity, const serial_bridge::Pose* pose) {
...
void ROSTempMotionController::sendJointState(serial_bridge::Motor* motor_left, serial_bridge::Motor* motor_right) {
...
pid_control_t ROSTempMotionController::get_pid(std::string name) {
    pid_control_t pid;
    double temp;
...
parameter_motors_t ROSTempMotionController::get_parameter() {
...
constraint_t ROSTempMotionController::get_constraint() {
...
bool ROSTempMotionController::pidServiceCallback(serial_bridge::Update::Request &req, serial_bridge::Update::Response&) {
...
bool ROSTempMotionController::parameterServiceCallback(std_srvs::Empty::Request&, std_srvs::Empty::Response&) {
...
bool ROSTempMotionController::constraintServiceCallback(std_srvs::Empty::Request&, std_srvs::Empty::Response&) {
    constraint_t constraint = get_constraint();
    try {
